...
class Collector:
    ...
    # Metodo para buscar a URL
    def get_content(self, **kwargs):
        resp = requests.get(self.url, params=kwargs)
        return resp
    
    # Metodos para salvar arquivos em json e parquet
    def save_parquet(self, data):
        now = datetime.datetime.now().strftime('%Y%m%d_%H%M%S.%f')
        df = pd.DataFrame(data)
        df.to_parquet(f'JovemNerd/data/{self.instance_name}/paquet/{now}.parquet', index=False)

    # função que invoca a função de metodos de salvamento dependendo do tipo de arquivo que será salvo
    ...
    # Conferencia se a API esta sendo responsiva para salvar os dados.
